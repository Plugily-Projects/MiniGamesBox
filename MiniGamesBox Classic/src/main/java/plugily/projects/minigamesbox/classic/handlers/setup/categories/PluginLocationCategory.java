...
public class PluginLocationCategory implements SetupCategoryHandler {
  ...
  public void addItems(NormalFastInv gui) {
    ...
    MaterialMultiLocationItem sign = new MaterialMultiLocationItem(
            setupInventory,
            new ItemBuilder(XMaterial.OAK_SIGN.parseMaterial()),
            "Game Sign",
            "Sign registered as Game Sign with join function",
            "signs",
            MaterialUtils.ALL_SIGNS,
            setupInventory.getPlugin().getConfigPreferences().getOption("BUNGEEMODE"), 0);
    itemList.add(sign);
