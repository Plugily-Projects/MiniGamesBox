    ...
    if(!registry.getPlugin().getConfigPreferences().getOption("BUNGEEMODE")) {
      registry.mapArgument(registry.getPlugin().getPluginNamePrefixLong(), new CommandArgument("randomjoin", "", CommandArgument.ExecutorType.PLAYER) {
        ...
        public void execute(CommandSender sender, String[] args) {
          ...
          List<PluginArena> arenas = registry.getPlugin().getArenaRegistry().getArenas().stream().filter(arena -> arena.getArenaState() == IArenaState.STARTING && arena.getPlayers().size() < arena.getMaximumPlayers()).collect(Collectors.toList());
          if(args.length > 1) {
